defmodule DiffieHellman do
  ...
  # ref1: https://www.geeksforgeeks.org/modular-exponentiation-power-in-modular-arithmetic
  # ref2: https://www.geeksforgeeks.org/modular-exponentiation-recursive

  defp pow_mod(0, _, _), do: 0
  defp pow_mod(_, 0, _), do: 1

  # pow is even -> (base ^ pow) % mod = ((base ^ pow/2) % mod * (base ^ pow/2) % mod) % mod
  defp pow_mod(base, pow, mod) when rem(pow, 2) == 0 do
    sub_result = pow_mod(base, div(pow, 2), mod)
    rem(sub_result * sub_result, mod)
  end

  # pow is odd -> (base ^ pow) % mod = ((base % mod) * (base ^ pow-1) % mod) % mod
  defp pow_mod(base, pow, mod) when rem(pow, 2) == 1 do
    base_mod = rem(base, mod)
    sub_result = pow_mod(base, pow - 1, mod)
    rem(base_mod * sub_result, mod)
  end
end
